...
/**
 * @author zapreevis
 * This class is responsible for deleting old chat messages
 */
public class RemoveOldChatMessagesExecutor extends QueryExecutor<Void> {
	...
	public static final int REMOVE_CHAT_MESSAGES_OLDER_THAN_MINUTES = 60;
	
	//Get the Log4j logger object
	private static final Logger logger = Logger.getLogger( RemoveOldChatMessagesExecutor.class );
	
	final int minutesOld;
	
	/**
	 * Delete the messages that are more than minutesOld minutes old
	 * @param minutesOld the number of minutes back in the past for which we keep messages intact
	 */
	public RemoveOldChatMessagesExecutor( final int minutesOld ){
		this.minutesOld = minutesOld;
	}

	public PreparedStatement prepareStatement(Connection connection) throws SQLException {
		final String updateQuery = "DELETE FROM " + CHAT_MESSAGES_TABLE + " WHERE " +
									SENT_DATE_FIELD_NAME_CHAT_MESSAGES_TABLE + "<?";
	...
	public ResultSet executeQuery(PreparedStatement pstmt, Void result) throws SQLException, SiteException {
		...
		logger.debug("We have removed " + deletedMsgsCount + " old chat messages from the database!" );
		return null;
	}

...
/**
 * @author zapreevis
 * This class is a dialog that allows user to manage the files that are attached to the forum messages
 */
public class ForumFilesManagerUI extends PagedActionGridDialog<ShortFileDescriptor> {
	
	private static final int NUMBER_OF_FILES_PER_PAGE = 7;
	private static final int MAXIMUM_NUMBER_OF_FILES = 10;
	private static final int NUMBER_OF_COLUMNS = 4;
	
	//The forum message we manage files for
	 ...
	 */
	void addNewForumMessageFile( final ShortFileDescriptor fileDesc ) {
		disableAllControls();
		this.forumMessage.attachedFileIds.add( fileDesc );
		this.updateActualData();
		//Indicate that there are new attached files
		setAttachedFilesIndicator( true );
		//Enable the controls
		enableAllControls();
	}
	
	/**
	 * Allows to set up the attached files indicator in the parent
	 * dialog if it is an instance of SendForumMessageDialogUI
	 * @param exist true if the files exist, false if not
	 */
	private void setAttachedFilesIndicator( final boolean exist) {
		//This dialog should be opened from SendForumMessageDialogUI but we check for it just in case
		if( parentDialog instanceof SendForumMessageDialogUI ) {
			//Update the uploaded files indicator, set it on
			( (SendForumMessageDialogUI) parentDialog ).setAttachedFiles( exist );
		}
	}
						...
						public void execute() {
							...
							CommStatusAsyncCallback<Void> callback = new CommStatusAsyncCallback<Void>(progressBarUI) {
								public void onSuccessAct(Void result) {
									...
									Iterator<ShortFileDescriptor> iter = forumMessage.attachedFileIds.iterator();
									while( iter.hasNext() ) {
										ShortFileDescriptor fileDesc = iter.next();
										if( deletedFileIds.contains( fileDesc.fileID ) ) {
											iter.remove();
										}
									}
									//If we deleted all files then update the parent dialog
									if( forumMessage.attachedFileIds.isEmpty() ) {
										setAttachedFilesIndicator( false );
									}
							...
							forumManagerObj.deleteFiles( SiteManager.getUserID(), SiteManager.getUserSessionId(),
		...
		addButton.addClickHandler( new ClickHandler() {
			public void onClick(ClickEvent e) {
				...
				( new SplitLoad( true ) {
					@Override
					public void execute() {
						ForumFileUploadDialogUI dialog = new ForumFileUploadDialogUI( thisDialog );
						dialog.show();
						dialog.center();
					}
				}).loadAndExecute();
			...
			public void onClick(ClickEvent e){
				...
				( new SplitLoad( true ) {
					...
					public void execute() {
						...
						ViewForumMediaFilesDialogUI viewDialog = new ViewForumMediaFilesDialogUI( fileDescriptor, ForumFilesManagerUI.this );
						viewDialog.show();
						viewDialog.center();
					}
				}).loadAndExecute();
	...
	@Override
	//We have to suppress warnings about custing to a generic type
	protected void browse( final int userID, final String userSessionID, final int forUserID, final int offset,
						   final int numberOfRowsPerPage, final AsyncCallback<OnePageViewData<ShortFileDescriptor>> callback) throws SiteException {
		final int maximumRequestedFileIdex = (offset + numberOfRowsPerPage);
		final int maximumAvailableFileIndex = this.forumMessage.attachedFileIds.size();
		...
		for( int index = offset; index < maximumFileIndex; index++ ) {
			pageData.entries.add( this.forumMessage.attachedFileIds.get(index) );
		}
	...
	@Override
	//We have to suppress warnings about custing to a generic type
	protected void count( final int userID, final String userSessionID, final int forUserID, final AsyncCallback<Integer> callback) throws SiteException {
		callback.onSuccess( new Integer( this.forumMessage.attachedFileIds.size() ) );
	}
	...
	protected void enableControls( boolean enable ) {
		...
		addButton.setEnabled( enable && ( this.forumMessage.attachedFileIds.size() < MAXIMUM_NUMBER_OF_FILES ) &&
	...
	protected void updateDialogTitle( int numberOfEntries, int numberOfPages, int currentPageNumber ) {
		this.setText( titlesI18N.forumMessageFilesDialogTitle( numberOfEntries, currentPageNumber, numberOfPages ) );
	}
